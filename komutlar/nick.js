...
exports.run = async (client, message, args) => {
  const emoji = client.emojis.find(emoji => emoji.name === "krmz");
  const emoji1 = client.emojis.find(emoji => emoji.name === "kitap");
  const emoji2 = client.emojis.find(emoji => emoji.name === "tik"); //TlhaMert Youtube Kanalı : https://youtube.com/c/TlhaMert

  if (
    !message.member.roles.has("756547812719591515") &&
  ...
  rochelle.setNickname(`୪ ${isim} | ${yas}`);

  message.react("698935589130731641");

  let embed = new Discord.RichEmbed()
    .setColor("#E0FFFF")
    .setTitle("୪ Melania", message.author.avatarURL)
    .setDescription(
      `**${emoji2} ${rochelle.user} Adlı Üyenin İsmi Başarıyla Kayıt Edildi** `
    )
    .setFooter("୪ Melania #Kingdom") //TlhaMert Youtube Kanalı : https://youtube.com/c/TlhaMert
    .setTimestamp();
