...
exports.run = (client, message, args) => {
  const müzisyen = message.guild.roles.find(r => r.id === "756548988613820551"); //buraya erkek rolünüzün id'sini koyun.
  ...
  if (
    !message.member.roles.array().filter(r => r.id === "756547812870848522") &&
    ...
    return message.channel.send(
      "Bu Komutu Kullanmak için** <@&758083700050231377> Rolüne Sahip Olman Lazım"
    );
    ...
    const embed = new Discord.RichEmbed()
      .setDescription(
        `**<@${c.user.id}>** adlı kişiye **<@&${müzisyen.id}>** rolü verildi. !`
      )
      .setColor("0x42fcd7");
    log.send(embed);
